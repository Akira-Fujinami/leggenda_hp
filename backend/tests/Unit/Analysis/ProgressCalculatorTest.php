...
class ProgressCalculatorTest extends TestCase
{
    ...
    {
        $jobs = collect([
            $this->job(JobType::FetchStaticPage, AnalysisJobStatus::Completed), // 10
            $this->job(JobType::FetchRobots, AnalysisJobStatus::Completed), // 4
            $this->job(JobType::RunLighthouse, AnalysisJobStatus::Running), // not counted
        ]);

        $this->assertSame(14, $this->calculator->forWebsiteAnalysis($jobs));
    }

    public function test_failed_jobs_still_count_toward_progress(): void
    {
        $jobs = collect([
            $this->job(JobType::FetchStaticPage, AnalysisJobStatus::Failed), // 10, counted as "done"
        ]);

        $this->assertSame(10, $this->calculator->forWebsiteAnalysis($jobs));
    }
