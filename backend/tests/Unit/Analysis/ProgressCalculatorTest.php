...
class ProgressCalculatorTest extends TestCase
{
    ...
    {
        $jobs = collect([
            $this->job(JobType::FetchStaticPage, AnalysisJobStatus::Completed), // 11
    ...
    {
        $jobs = collect([
            $this->job(JobType::FetchStaticPage, AnalysisJobStatus::Failed), // 11, counted as "done"
        ]);

        $this->assertSame(11, $this->calculator->forWebsiteAnalysis($jobs));
    }
