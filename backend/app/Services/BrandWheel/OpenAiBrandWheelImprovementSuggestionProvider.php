...
class OpenAiBrandWheelImprovementSuggestionProvider implements BrandWheelImprovementSuggestionProvider
{
     ...
     *
     * v13(2026-08-28、依頼AO): focus_items_reasonの中身の質を改善した。
     * 実物レポート54の検証で、生成された理由がtype(catch_up/breakout)の
     * ラベル ―― 「競合が既に情報を提供しているため」「競合が伝えている
     * 情報であり」 ―― をそのまま言い換えているだけで、同じページの
     * カード自体が既に言っていることの再掲になっていた(かつ①のカード
     * すべてに等しく当てはまる一般論のため、自社に無い項目9件のうち
     * 「なぜこの3件なのか」という選定の理由になっていなかった)。加えて
     * 「自由度を理解するために重要」のような下位要素名の言い換えに
     * すぎない同語反復も確認された。同じページの中長期の差別化ポイント
     * (mid_term_action)は、self_matched_items/self_key_message/
     * self_core_value_evidence等、自社サイトから実際に読み取れたことに
     * 接続しているため、その会社にしか当てはまらない文になっており
     * 明らかに質が高かった ―― この作り方を理由にもそのまま適用した。
     * 【禁止: カードのラベルの再掲】を新設し、typeの意味をそのまま言い換える
     * ことを明示的に禁止したうえで、self_matched_items(evidence付き)・
     * self_key_message・self_positive_impression・self_core_value_evidence
     * を材料に「自社に既にあるものとの関係でこの項目を説明する」よう
     * 指示した(データ自体はv4の差別化テーマ選定用にInput/InputFactory側で
     * 既に用意済みのものを流用しており、新しい入力フィールドは追加して
     * いない)。また、3件すべてに均等配分することを要件から外し(材料が
     * 乏しい項目は無理に埋めず触れなくてよい)、ただしfocus_items_for_reason
     * [0](one_pointで名指しした項目=カード1)には必ず触れることを明示した
     * (依頼者指定 ―― 埋めるために同語反復を作るくらいなら書ける項目に
     * 文字数を使うほうがよい)。①②の書き分け(依頼AH-3)は維持しつつ、
     * ラベルの再掲ではなく「そこから何が言えるか」(候補者の比較検討時に
     * 不利に働きうる/先に言えば差がつきうる)を書くよう表現を改めた。
     * 語尾の反復禁止(依頼Q-2)・分量(3〜4文、パーサ側の文字数上限は無変更)・
     * 断定禁止・出力言語・スキーマ(フィールド名・型)は変更していない。
     * focus_sub_element_keys・one_point等、focus_items_reason以外の
     * フィールドの指示・判定ロジック(項目の選定はPHP側に一元化されたまま)・
     * mid_term_actionの生成条件はv12から無変更 ―― mid_term_actionの
     * 品質を劣化させないことが依頼者の明示的な要求のため、その指示文
     * ($gapDifferentiationNote等)には一切手を入れていない。
     *
     * v14(2026-08-28、依頼AP): v13の検証(依頼AO-2、実際にAPIを呼んだ比較)で
     * 使ったテスト入力自体に、24項目に存在しない架空の項目名
     * (「事業運営スタイル」、config('brand_wheel.axes.personality.sub_elements')
     * には存在しない)が誤って含まれていた。AIはこの架空の名前を入力データ
     * から忠実にそのまま引用しただけだったが、この事故を機に、AIが
     * axis_name(軸名)とsub_name(項目名)を混ぜて新しい名前を合成する
     * 一般的なリスク自体は残る(生成が確率的である以上、正しい入力でも
     * いずれ起こりうる、依頼者指摘)と判断し、2点対応した。
     * (1)【項目名の書き方(最重要)】を新設 ―― 自社の項目に言及する際は
     * self_matched_items/self_unmatched_items/focus_items_for_reasonの
     * sub_nameを要約・言い換え・合成せずそのまま使うこと、項目名として
     * 言及する場合のみ『』で囲むこと、サイトからの引用(evidence等)は
     * 項目名ではないため『』で囲まないことを明示した。
     * (2)機械的な検知(BrandWheelReasonBracketNameValidator、新規)を
     * GenerateBrandWheelImprovementSuggestionJob側に追加した ―― 生成された
     * focus_items_reason中の『』の中身を、24項目(sub_elements)・6カテゴリ
     * (axes.*.name_ja)・3領域(group_labels)の正式名称と照合し、
     * いずれにも一致しない文字列が1つでもあれば、focus_items_reasonを
     * null化してwarningログ(analysis_id・invalid_names、本文は含めない)を
     * 出す(依頼AF-2/AA-3と同じ「レポート生成自体は失敗させない」方針)。
     * プロンプトの指示だけに頼らない多層防御とする。
     *
     * 加えて、依頼AOでfocus_items_reasonの分量指示を「最大3文」から
     * 「3〜4文」に緩めたが、パーサ側の文字数上限(FOCUS_ITEMS_REASON_MAX_CHARS
     * =200)は無変更のままだったため、4文になると文の境界で切られ
     * (BrandWheelTextTruncator::truncateAtSentenceBoundary())、最後の1文
     * (実質的にカード3についての理由)がまるごと欠落する事故が起こりうる
     * ことが判明した(依頼AOの実測で3文=187字、200字までの余裕が僅か
     * 13字しかないことを確認済み)。「材料が乏しい項目は無理に埋めない」
     * という依頼AOの狙いは、3文のままでも(実際、材料が乏しい入力では
     * 依頼AOの実測で2文しか生成されなかった)成立するため、200字上限は
     * 変更せず、指示を「最大3文」に戻した(依頼者提示の2択のうち、
     * レイアウトへの影響が無い側を選択)。
     *
     * 併せて、切り詰めが起きたこと自体を沈黙させないよう、
     * BrandWheelImprovementSuggestionResponseParserにtruncatedFields
     * (どのフィールドが何字→何字に切り詰められたか)を追加し、
     * GenerateBrandWheelImprovementSuggestionJob側でwarningログ
     * (analysis_id・field・original_chars・truncated_chars、本文は
     * 含めない)を出すようにした ―― one_point/recommendation/reason/
     * mid_term_action/focus_items_reasonの5つの自由記述フィールド全てに
     * 適用する(同じtruncateAtSentenceBoundary()を通る箇所であり、
     * 機構を1つ作れば追加コストなく揃えられるため)。recommended_contents
     * (箇条書き、1件45字上限)には適用していない ―― 1件が短い定型フレーズで
     * あり、複数文にまたがる理由付けが丸ごと欠落するreason/focus_items_reason
     * 等とはリスクの性質が異なると判断したため(必要になれば改めて追加を
     * 検討する)。
     *
     * 断定禁止・出力言語・語尾の反復禁止・mid_term_actionの生成条件・
     * ①②の書き分け・「材料が乏しい項目は無理に埋めない」「カード1には
     * 必ず触れる」は変更していない。
     */
    public const string PROMPT_VERSION = 'v14';
...
各要素のtypeは、その項目の性格を示します:
- type="catch_up"(①追いつく): 競合サイトには記述があり、自社サイトには無い項目。
- type="breakout"(②抜け出す): 競合サイトにも自社サイトにも記述が無い項目。

【禁止: カードのラベルの再掲(最重要)】
上記のtypeの意味 ―― 「競合は既に伝えている」「まだどちらも伝えていない」 ――
は、改善提案ページのカード自体に既に表示されています。この事実をそのまま
言い換えただけの文(「競合が既に情報を提供しているため」「競合が伝えている
情報であり」等)を理由として書かないでください。これはtypeが同じ項目すべてに
等しく当てはまる一般論であり、「なぜ他の項目ではなくこの項目なのか」という
選定の理由になっていません。下位要素名を言い換えただけの説明(例:「自由度を
理解するために重要」)も同様に禁止です。

理由が答えるべき問いは、「自社に無い項目(self_unmatched_items、通常複数件
ある)のうち、なぜこの項目が優先されるのか」です。self_matched_items(自社で
既に確認できている項目、evidence付き)・self_key_message・
self_positive_impression・self_core_value_evidence(あれば)を材料にし、
「御社は既に○○を伝えているが、それを支える△△がまだ書かれていない」のように、
自社に既にあるものとの関係でこの項目を説明してください。これはmid_term_action
と同じ作り方です ―― その会社のサイトから実際に読み取れたことに接続し、他社の
レポートにそのまま貼り付けられない、その会社にしか当てはまらない文にして
ください。

typeによる性格の違いは、ラベルの再掲ではなく、そこから何が言えるかとして
反映してください:
- type="catch_up": 候補者が複数社を比較検討する場面で、この情報が無いことが
  不利に働きうる、という観点。
- type="breakout": 先に伝えることで候補者への訴求で差がつきうる、という観点。
  「両社とも書いていないので差別化できます」のような消極的な理由付けは
  禁止です。
複数の項目が混在する場合は、それぞれの性格の違いに触れながら1つの理由文として
まとめてください(catch_up/breakoutで別々の理由文に分けなくてよい)。

【材料が乏しい項目は無理に埋めない】
focus_items_for_reasonの各項目に均等に1文ずつ割り当てる必要はありません。
自社の確認済み項目との接続が見出せない項目は、無理に理由を書かず触れなくて構いません
(同語反復や一般論で埋めるより、書ける項目に文字数を使ってください)。
ただしfocus_items_for_reason[0](one_pointで名指しした項目)には必ず触れてください。

【項目名の書き方(最重要: 正式名称をそのまま使うこと)】
自社の項目(self_matched_items・self_unmatched_items・focus_items_for_reason)に
言及する際は、それぞれのsub_nameの値を、要約・言い換え・合成せずそのまま使って
ください。axis_name(軸名、例:「経営スタイル」)と別の項目のsub_nameを組み合わせて
新しい名前を作ること(存在しない名前の合成)は禁止です。項目名として言及する場合
だけ『』で囲んでください。サイトから引用した記述(evidence等)は項目名ではないため、
『』で囲まないでください ―― 項目名と引用が同じ記号だと、読み手が24項目の対比表で
探しても見つからず混乱します。
...
- focus_items_reason: 最大3文で、data.focus_items_for_reasonに列挙された項目(だけ)について、
  なぜ最優先で改善すべきかを説明してください。【禁止: カードのラベルの再掲】の指示どおり、
  typeの意味(競合が既に伝えている/まだどちらも伝えていない)をそのまま言い換えるのではなく、
  self_matched_items・self_key_message・self_positive_impression・self_core_value_evidence等、
  自社に既にある情報との関係でこの項目を説明してください。項目名は【項目名の書き方】の指示どおり
  sub_nameの値をそのまま使い、要約・言い換え・合成をしないでください。focus_items_for_reason[0]
  (one_pointで名指しした項目)には必ず触れてください。自社の確認済み項目との接続が見出せない
  項目は無理に埋めず、触れなくて構いません。実際のデータに基づいて具体的に書き、【断定禁止】の
  指示に従い条件付き表現にしてください。複数文にわたるため、【文章の書き方】の指示どおり同じ
  語尾を2回以上繰り返さないよう、文ごとに表現を変えてください。focus_items_for_reasonが
  空配列の場合はJSONのnull(文字列"null"ではない)にしてください。
TXT;
