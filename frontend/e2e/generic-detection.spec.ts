...
test("Fixture A: H1・SNS・代替サポート・価格付き商品カードが矛盾なく表示される", async ({ page }) => {
  ...
  // (ヘルプ導線があるため優先度が下がった提案として表示される)。結果画面の
  // 優先改善は上位3件のみ、比較画面の改善提案も既定では上位5件のみのため、
  // 「すべての改善提案を見る」を開いて全件表示の改善提案一覧で確認する。
  const comparisonUrl = page.url().replace(/\/results.*$/, "/comparison");
  await page.goto(comparisonUrl);
  // 改善提案データの読み込み完了を待ってから「すべて見る」の有無を判定する
  // (isVisible()は自動リトライしないため、読み込み中の一瞬を見て
  // 「無い」と誤判定しないよう、先にプレビューの見出し表示を待つ)。
  await expect(page.getByText("ルールベース改善提案")).toBeVisible();
  const seeAllButton = page.getByRole("button", { name: /すべての改善提案を見る/ });
