...
**2026-08-24追記(主因を特定)**: #99の実データ検証で、同一サイト・同一入力でも`matched`数が3/0/3のように変動する事例を追跡したところ、開発環境の既定モデル`gpt-4o-mini`が、プロンプト中の下位要素定義文をそのままevidenceとして返す(サイト本文を引用しない)挙動を高頻度で起こすことが主因と判明した。本番モデル`gpt-4o`では同一条件を3回実行して安定(8/8/8)し、この挙動は再現しなかった。**実データ検証は必ず本番と同じモデル(`BRAND_WHEEL_AI_MODEL=gpt-4o`、`.env.example`参照)で行うこと ―― モデルが違えば結果は比較できない。** この失敗パターンは`BrandWheelDiscardedSubElement`の`definition_echo`理由で観測できる(2026-08-24追加)。

**2026-08-24追記2(訂正: gpt-4oも「完全に安定」ではない)**: 上記の「gpt-4oでは完全に安定」は3回サンプルの範囲内での観測であり、誤りだった。同じサイト(`quil-fait-bon-recruit.com`)・同じv8プロンプト(1文字も変更していないことを`git diff`で確認済み)で、数時間後に改めて3回実行したところ`matched`は9, 9, 8となり、以前記録した7, 7, 7とは一致しなかった。6回分をまとめて項目キー単位で比較すると、中心となる7項目(`will_activity.business_expansion`/`asset.scale_influence`/`personality.company_character`/`personality.core_values`/`relationship.colleagues`/`relationship.atmosphere`/`financial_benefit.salary_level`)は6回すべてで一致しており、揺れているのは境界線上の`emotional_benefit.pride`/`emotional_benefit.satisfaction`の2項目のみだった。バッチ内(3回)では一致し、バッチ間(数時間空けた別の3回)で異なるという出方は、OpenAI側の配信バックエンドが時間経過で変わり`system_fingerprint`が変化する挙動と整合する(未検証、記録として残すのみ)。

**対処しない判断(2026-08-24)**: 上記の揺れについて、現時点では対処しないと判断した。理由: (1) 中心の項目は動いておらず、診断の結論(どの領域が弱いか)自体は変わらない、(2) レポートは点数ではなく件数のみを表示しており24項目中7か9かで顧客向けの説明は変わらない、(3) 一度生成したレポートは保存され、再ダウンロードしても内容は変わらない(input_hashによる再利用キャッシュのため、揺れが表面化するのは診断をやり直した場合のみ)、(4) 多数決(複数回実行して最頻値を採る)はコスト増(3倍等)に見合わない。将来、この揺れが実害(顧客からの指摘等)につながった場合に再検討する。
